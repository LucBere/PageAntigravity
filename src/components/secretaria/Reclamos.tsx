...
export default function Reclamos() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-8 pt-12 pb-12 min-h-full bg-[#0E0E0E] relative">
      {/* 1. Encabezado */}
      <div>
      ...
      <div className="bg-[#151515] rounded-2xl border border-zinc-800/50 p-6">
          ...
          <table className="w-full text-left border-collapse min-w-max">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {reclamos.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="py-8 text-center text-zinc-500 text-sm font-bold uppercase tracking-widest">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between pt-6 mt-2 border-t border-zinc-800/50">
          ...
          <div className="flex items-center space-x-2 mt-4 md:mt-0">
            <button className="p-2 rounded-lg bg-[#1A1A1A] border border-zinc-800 text-zinc-500 hover:text-white transition-colors cursor-pointer disabled:opacity-50" disabled>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        ...
        <div className="bg-[#151515] rounded-xl p-6 border border-zinc-800/50 border-l-2 border-l-red-500 relative overflow-hidden transition-all">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">PENDIENTES HOY</h3>
            <ClipboardList className="w-5 h-5 text-red-500" />
          </div>
          <p className="text-5xl font-normal text-white mb-2 tracking-tight">
      ...
      {reclamoSeleccionado && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-[#151515] border border-zinc-800 rounded-2xl p-8 max-w-lg w-full shadow-2xl shadow-black">
            
            <div className="mb-6 text-center">
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-2">VERIFICACIÓN DE COMPROBANTE</p>
              <h2 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight">{reclamoSeleccionado.name}</h2>
              <p className="text-xs text-zinc-400 mt-1">DNI: {reclamoSeleccionado.dni}</p>
            </div>

            {/* Placeholder de la Imagen del Comprobante */}
            <div className="w-full h-64 bg-[#1A1A1A] border border-zinc-800 rounded-xl mb-8 flex flex-col items-center justify-center text-zinc-500 overflow-hidden relative">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-zinc-600">Comprobante_Transferencia.jpg</p>
            ...
            <div className="flex gap-4">
              <button 
                ...
                className="flex-1 py-4 border-2 border-red-900/30 hover:border-red-500 text-red-500 hover:text-red-400 text-xs font-bold tracking-widest uppercase rounded-xl transition-all"
              >
                RECHAZAR
              </button>
              <button 
            ...
            <div className="mt-4 text-center">
              <button 
                ...
                className="py-2 px-4 text-zinc-500 hover:text-white text-[10px] font-bold tracking-widest uppercase transition-colors"
              >
                CERRAR SIN CAMBIOS
              </button>
            </div>
