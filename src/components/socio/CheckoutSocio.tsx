  ...
  return (
    <div className="max-w-7xl mx-auto pb-12 relative">
      ...
      <div className="mb-8 hidden md:block">
        <h1 className="text-[11px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-[0.3em]">CHECKOUT SQUATGYM</h1>
      </div>
            ...
            <form onSubmit={handleConfirm} className="space-y-6 relative z-10">
              ...
              {paymentMethod === 'transferencia' && (
                <div className="bg-slate-50 dark:bg-[#1A1A1A] border border-slate-200 dark:border-zinc-800 rounded-xl p-6 text-center space-y-4">
                  ...
                  <div className="space-y-1 pt-4 border-t border-slate-200 dark:border-zinc-800">
                    <p className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest">ALIAS</p>
                    <p className="text-lg font-bold text-slate-900 dark:text-white tracking-widest">SQUAT.ELITE.GYM</p>
                  </div>
              ...
              {paymentMethod === 'qr' && (
                <div className="bg-slate-50 dark:bg-[#1A1A1A] border border-slate-200 dark:border-zinc-800 rounded-xl p-8 flex flex-col items-center text-center space-y-6">
                  <div className="bg-white p-4 rounded-xl">
                    <QrCode className="w-48 h-48 text-[#0E0E0E]" />
                  </div>
                  <p className="text-sm font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">Escanea el código desde tu app<br/>de pago favorita</p>
                </div>
              )}

              <button type="submit" disabled={!isFormValid() || isProcessing} className={`w-full py-5 rounded-2xl text-lg font-black uppercase tracking-[0.2em] transition-all duration-300 mt-8 ${!isFormValid() ? 'bg-slate-100 dark:bg-zinc-800 text-slate-400 dark:text-zinc-600 cursor-not-allowed' : 'bg-[#388E3C] hover:bg-emerald-600 text-[#0E0E0E] shadow-lg shadow-[#388E3C]/20 cursor-pointer'}`}>
                {isProcessing ? 'PROCESANDO...' : 'CONFIRMAR PAGO'}
              </button>

              <div className="flex items-center justify-center space-x-2 mt-6 text-slate-400 dark:text-zinc-600">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-[#151515] p-8 rounded-3xl border border-slate-200 dark:border-slate-200 dark:border-zinc-800/50 transition-colors shadow-sm dark:shadow-none">
            ...
            <div className="space-y-6 mb-8">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">PLAN {selectedPlan.toUpperCase()}</span>
                <span className="text-sm font-bold text-slate-900 dark:text-white tracking-wider">${basePrice.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">INSCRIPCIÓN</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-slate-900 dark:text-white tracking-wider">${iva.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
            </div>
            ...
            <div className="border-t border-slate-300 dark:border-slate-200 dark:border-zinc-800/80 pt-8 flex items-center justify-between">
              ...
              <div className="text-right flex flex-col items-end">
                <span className="text-4xl font-black text-[#388E3C] tracking-tighter flex items-end">
                  ${total.toLocaleString(undefined, {minimumFractionDigits: 2})} <span className="text-[10px] font-bold text-slate-500 dark:text-zinc-500 uppercase ml-2 tracking-widest mb-2">ARS</span>
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-zinc-800/80 to-[#151515] p-8 rounded-3xl border border-slate-300 dark:border-zinc-700/50 relative overflow-hidden flex flex-col items-center text-center mt-6">
            ...
            <div className="relative z-10 py-4">
              ...
              <p className="text-[9px] text-slate-500 dark:text-zinc-400 font-bold uppercase tracking-[0.2em] leading-relaxed max-w-[200px] mx-auto">ACCESO TOTAL 24/7 EN TODAS LAS SUCURSALES DEL PAÍS</p>
              <h4 className="text-xl font-black text-slate-400 dark:text-zinc-600 uppercase tracking-widest mt-6 opacity-30">SAFEE WORK</h4>
            </div>
          </div>
        </div>
      </div>
